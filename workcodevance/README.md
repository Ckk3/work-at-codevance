...
Project made to apply to python jobs
...
pyenv virtualenv 3.9.8 workcodevance
...
```
cd
git clone git@github.com:moacirmoda/workcodevance .git git
```
...
```bash
heroku login
heroku create workcodevance-production

heroku config:set SECRET_KEY='---mY-SEcReT---'
heroku config:set DEBUG=false
heroku config:set ALLOWED_HOSTS=*
heroku config:set DATABASE_URL=postgres://workcodevance:workcodevance@localhost:5432/workcodevance
...
heroku config:set AWS_STORAGE_BUCKET_NAME=workcodevance-static
